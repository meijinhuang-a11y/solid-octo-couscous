...
export default function Sidebar({ isOpen, onClose, onSettingsClick, isMobile = false }: SidebarProps) {
  ...
  const sidebarContent = (
    <>
      ...
      <div
        ...
        style={{ height: '56px', borderBottom: '1px solid var(--sidebar-border)' }}
      >
        ...
        {orderedItems.map((item) => {
          ...
          return (
            <NavLink
              ...
              onClick={onClose}
              className={`flex items-center gap-2.5 rounded-lg truncate transition-all duration-150
                ${isActive ? '' : 'hover:bg-sidebar-accent hover:pl-3'}`}
              style={{
                padding: isActive ? '9px 12px' : '9px 14px',
                fontFamily: 'var(--font-sans)',
                fontSize: '0.8125rem',
                fontWeight: 500,
      ...
      <div
        ...
        style={{ padding: '12px 16px', borderTop: '1px solid var(--sidebar-border)' }}
      >
        <button
          type="button"
  ...
  return (
    <aside
      className="fixed left-0 top-0 bottom-0 z-50 flex flex-col w-60"
      style={{
        backgroundImage:
          'linear-gradient(180deg, var(--sidebar) 0%, color-mix(in srgb, var(--sidebar) 95%, var(--primary)) 100%)',
        borderRight: '1px solid var(--sidebar-border)',
